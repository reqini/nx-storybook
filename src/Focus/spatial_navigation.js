...
;(function($) {
  ...
  function parseSelector(selector) {
    ...
    if ($) {
      ...
      //#100-Adrenalina .focusable
      try {
        result = [].slice.call(document.querySelectorAll(selector));
      } catch(e) {
        var array_selector=selector.split(' ');
        var string_result=[];
        var id_result=[];
        for(var ax in array_selector){
          var t=array_selector[ax];
          if(t.indexOf('#')!==-1){
            id_result.push(t.replace("#",""));
          }else{
            string_result.push(t);
          }
        }
        var s_end=id_result.join("");
        //console.log("[id='",id_result.join("").replace(" "),"","'] .focusable");
        result = [].slice.call(document.querySelectorAll("[id='"+s_end+"'] "+string_result.join(" ")));
      }
  ...
  function onKeyDown(evt) {
    // console.log('Listening Spatial');

    switch (true) {
        case (evt.keyCode === 37 && evt.shiftKey === true) :
        case (evt.keyCode === 39 && evt.shiftKey === true) :
        case (evt.keyCode === 38 && evt.shiftKey === true) :
        case (evt.keyCode === 40 && evt.shiftKey === true) :
        case (evt.keyCode === 87 && evt.shiftKey === true) :
        case (evt.keyCode === 81 && evt.shiftKey === true) :
        case (evt.keyCode === 427 && evt.shiftKey === true) :
        case (evt.keyCode === 428 && evt.shiftKey === true) :
            evt.preventDefault(); evt.stopPropagation();
            virtualKey(evt.keyCode);
            return;
        default:
        break;
    }
