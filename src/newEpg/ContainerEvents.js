...
const CollectionExample = ({
  ...
  const [positionCellChannel, setPositionCellChannel] = useState(
    get(currentEvent, "channel.number", 1)
  ); // donde posiciona el foco de channels

  const [positionCell, setPositionCell] = useState(
    get(currentEvent, "", { index: 1 })
  ); // donde posiciona la grilla(nro de index) y datos del evento activo
        ...
        if (!miniEpg) {
          if (resultTop === 0) {
            ...
            refScroll.current._onScroll({
              ...scroll,
              scrollTop:
                style.top - epgFullHeight < 0 ? 0 : style.top - epgFullHeight,
            });
          }
        }
      }
  ...
  const onFocus = useCallback(({ event, style, miniEpg }) => {
    ...
    if (
      (!event.last &&
        (style.left < scroll.scrollLeft || result > scroll.clientWidth)) ||
          ...
          <div>
            ...
            >
              ...
              <Collection
                ...
                scrollToAlignment={
                  miniEpg ? "start" : resetPosition ? "start" : "auto"
                }
